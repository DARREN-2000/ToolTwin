    ...
    async function runSimulation() {
      ...
      try {
        ...
        const mockSimulation = {
          ...
          dependency_graph: {
            nodes: [
              { id: '1', type: 'custom', position: { x: 250, y: 0 }, data: { label: 'delete_customer', icon: 'action', description: 'CUS-10482' } },
              { id: '2', type: 'custom', position: { x: 100, y: 150 }, data: { label: 'orders', icon: 'database', description: 'Cascading delete 3 records' } },
              { id: '3', type: 'custom', position: { x: 400, y: 150 }, data: { label: 'support_tickets', icon: 'database', description: 'Cascading delete 2 records' } },
              { id: '4', type: 'custom', position: { x: 100, y: 300 }, data: { label: 'payments', icon: 'database', description: 'Cascading delete 3 records' } },
              { id: '5', type: 'custom', position: { x: 250, y: 300 }, data: { label: 'analytics_aggregates', icon: 'warning', description: 'Orphaned metrics' } }
            ],
            edges: [
              { id: 'e1-2', source: '1', target: '2', animated: true, style: { stroke: 'hsl(var(--accent))', strokeWidth: 2 } },
              { id: 'e1-3', source: '1', target: '3', animated: true, style: { stroke: 'hsl(var(--accent))', strokeWidth: 2 } },
              { id: 'e2-4', source: '2', target: '4', animated: true, style: { stroke: 'hsl(var(--destructive))', strokeWidth: 2 } },
              { id: 'e1-5', source: '1', target: '5', animated: true, style: { stroke: 'hsl(var(--destructive))', strokeWidth: 2 } }
            ]
          }
        };
        ...
        toast.success("Loaded simulation (Demo Mode)");
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="h-full flex items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-4 text-primary">
          ...
          <p className="font-heading font-medium animate-pulse">
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      <div className="p-4 md:p-6 border-b border-border flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-muted/20">
        <div>
          <h1 className="text-xl font-heading font-bold text-foreground break-all">
        ...
        <div className="flex gap-2">
          <span
            className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 ${simulation.risk_level === "CRITICAL" ? "badge-risk-critical" : simulation.risk_level === "HIGH" ? "badge-risk-high" : simulation.risk_level === "MEDIUM" ? "badge-risk-medium" : "badge-risk-low"}`}
          >
            <ShieldAlert className="w-3 h-3" /> {simulation.risk_level} RISK (
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        {/* Graph Area */}
        <div
          className={`flex-1 relative transition-all duration-300 border-b lg:border-b-0 lg:border-r border-border ${selectedNode ? "lg:mr-96" : ""}`}
        >
          <ReactFlow
            nodes={nodes}
            edges={edges}
            ...
            <div className="flex flex-col h-full bg-background/50 backdrop-blur-sm z-10 p-4 md:p-6 overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="bg-muted p-4 rounded-lg">
                  <p className="text-xs text-foreground/50 uppercase font-semibold mb-1">
                    Node ID
                  </p>
            ...
            <div className="h-full p-4 md:p-6 overflow-y-auto space-y-6">
              <div className="space-y-2">
                <h3 className="font-semibold text-foreground flex items-center gap-2">
                  <AlertTriangle
                    className={
                      simulation.policy_passed
                        ? "text-accent w-5 h-5"
                        : "text-destructive w-5 h-5"
                    }
                  />
              ...
              {simulation.alternatives?.length > 0 && (
                <div className="space-y-2">
                  <h3 className="font-semibold text-foreground flex items-center gap-2">
                    <CheckCircle className="text-accent w-5 h-5" /> Recommended
                    Alternative
                  </h3>
                  {simulation.alternatives.map((alt: any, i: number) => (
                    <div
                      key={i}
                      className="bg-accent/10 border border-accent/30 rounded-lg p-4 space-y-3"
                    >
