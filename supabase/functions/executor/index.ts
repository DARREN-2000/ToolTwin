  ...
  try {
    ...
    if (tool_name === "delete_customer" && tool_params?.customer_id) {
      const { error: pErr } = await supabase
        ...
        .from("payments")
        .delete()
        .eq("order_id", tool_params.customer_id);
      ...
      const { error: oErr } = await supabase
        ...
        .from("orders")
        .delete()
        .eq("customer_id", tool_params.customer_id);
      ...
      if (error)
        throw new Error(`Failed to anonymize customer: ${error.message}`);
      message = `Successfully anonymized customer ${tool_params.customer_id}.`;
    } else {
      return new Response(
        JSON.stringify({ error: "Invalid tool_name or missing parameters" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const cryptographic_signature = hashArray
